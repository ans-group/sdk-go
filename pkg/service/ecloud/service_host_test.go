	...
	t.Run("Single", func(t *testing.T) {
		...
		c.EXPECT().Get("/ecloud/v2/hosts", gomock.Any()).Return(&connection.APIResponse{
			Response: &http.Response{
				Body:       io.NopCloser(bytes.NewReader([]byte("{\"data\":[{\"id\":\"h-abcdef12\"}],\"meta\":{\"pagination\":{\"total_pages\":1}}}"))),
				StatusCode: 200,
			},
		}, nil).Times(1)
...
func TestGetHost(t *testing.T) {
	t.Run("Valid", func(t *testing.T) {
		...
		c.EXPECT().Get("/ecloud/v2/hosts/h-abcdef12", gomock.Any()).Return(&connection.APIResponse{
			Response: &http.Response{
				Body:       io.NopCloser(bytes.NewReader([]byte("{\"data\":{\"id\":\"h-abcdef12\"}}"))),
				StatusCode: 200,
			},
		}, nil).Times(1)
...
func TestCreateHost(t *testing.T) {
	t.Run("Valid", func(t *testing.T) {
		...
		c.EXPECT().Post("/ecloud/v2/hosts", &req).Return(&connection.APIResponse{
			Response: &http.Response{
				Body:       io.NopCloser(bytes.NewReader([]byte("{\"data\":{\"id\":\"h-abcdef12\",\"task_id\":\"task-abcdef12\"}}"))),
				StatusCode: 202,
			},
		}, nil).Times(1)

		taskRef, err := s.CreateHost(req)

		assert.Nil(t, err)
		assert.Equal(t, "h-abcdef12", taskRef.ResourceID)
...
func TestPatchHost(t *testing.T) {
	t.Run("Valid", func(t *testing.T) {
		...
		taskRef, err := s.PatchHost("h-abcdef12", req)
	...
	t.Run("ConnectionError_ReturnsError", func(t *testing.T) {
		...
		_, err := s.PatchHost("h-abcdef12", PatchHostRequest{})
	...
	t.Run("InvalidHostID_ReturnsError", func(t *testing.T) {
		...
		_, err := s.PatchHost("", PatchHostRequest{})
...
func TestDeleteHost(t *testing.T) {
	t.Run("Valid", func(t *testing.T) {
		...
		taskID, err := s.DeleteHost("h-abcdef12")
